...
- [Tests/QTTtabBarTests/ArchitectureHotspotBudgetTests.cs](file://Tests/QTTtabBarTests/ArchitectureHotspotBudgetTests.cs)
- [Tests/QTTtabBarTests/StructuralGovernanceBaselineTests.cs](file://Tests/QTTtabBarTests/StructuralGovernanceBaselineTests.cs)
- [docs/superpowers/plans/2026-07-10-structural-governance-remediation.md](file://docs/superpowers/plans/2026-07-10-structural-governance-remediation.md)
...
## 更新摘要
**变更内容**
- 新增架构治理基础设施，包括 ArchitectureHotspotBudgetTests 防止架构债务增长
- 新增 StructuralGovernanceBaselineTests 建立基线测量和监控机制
- 新增 GitHub Actions 工作流自动执行架构测试的规划文档
- 强化架构质量门禁，确保代码重构不会导致架构债务累积
...
30. [架构治理基础设施](#架构治理基础设施)
31. [安装程序互操作性测试](#安装程序互操作性测试)
32. [特定回归测试](#特定回归测试)
33. [手动验证工具](#手动验证工具)
34. [构建系统改进](#构建系统改进)
35. [持续集成与自动化](#持续集成与自动化)
36. [调试与故障排除](#调试与故障排除)
37. [贡献指南](#贡献指南)
38. [结论](#结论)
...
**更新** 新增了全面的架构治理基础设施，包括架构热点预算测试和结构治理基线测试，专门防止架构债务增长和建立基线测量机制。这些测试确保所有架构重构的正确性和向后兼容性，维持 687/687 绿色测试基线。新增的架构治理基础设施涵盖了代码复杂度控制、嵌套控制器数量限制、所有者回引用计数等关键架构指标的监控。
...
- 测试（QTTtabBarTests）：NUnit 单元测试与安全回归测试，包含契约测试、重构保护、会话状态竞态条件验证、线程安全测试、facade等价性测试、TabManager重构测试、架构批次测试、架构治理测试、架构审查修复测试、安装程序互操作性测试和特定回归测试。

**更新** 测试套件现在包含专门的架构治理基础设施，包括架构热点预算测试和结构治理基线测试，用于监控和防止架构债务增长。这些测试通过量化指标如代码行数、嵌套控制器数量、所有者回引用计数等来确保架构质量的持续改进。
...
S --> T["验证架构债务不增长"]
T --> U["运行架构审查修复测试"]
U --> V["验证IPC版本/主题同步/初始化重试"]
V --> W["运行安装程序互操作性测试"]
W --> X["验证Explorer重启安全/GAC部署/单实例升级"]
X --> Y["运行特定回归测试"]
Y --> Z["验证标签文本布局/字段阴影"]
```
...
- **新增** 架构治理测试验证架构债务不增长，包括代码行数、嵌套控制器数量、所有者回引用计数等指标。
- **新增** 安装程序互操作性测试验证Explorer重启安全性、GAC部署安全性和单实例升级行为。
- **新增** 特定回归测试验证标签文本布局和字段阴影问题的修复。

```mermaid
classDiagram
class ArchitectureHotspotBudgetTests {
+NoGrowth_Budget_Is_Not_Exceeded()
+Final_Budget_QTTabBarClass_FileLines()
+Final_Budget_QTTabBarClass_NestedControllers()
+Final_Budget_QTTabBarClass_OwnerBackReferences()
+Final_Budget_QTButtonBar_FileLines()
+Final_Budget_OptionsDialog_FileLines()
}
class StructuralGovernanceBaselineTests {
+Current_Debt_Baseline_Is_Not_Exceeded()
+Architecture_Plan_Reopens_W10_And_C6()
+SourceMetrics_FamilyLines()
+SourceMetrics_NestedControllerCount()
+SourceMetrics_TokenCount()
}
...
QTTtabBarTests_csproj --> ArchitectureHotspotBudgetTests : "包含"
QTTtabBarTests_csproj --> StructuralGovernanceBaselineTests : "包含"
```
...
- **新增** 架构治理测试：验证架构债务不增长，包括代码行数、嵌套控制器数量、所有者回引用计数等指标的监控。
- **新增** 安装程序互操作性测试：验证Explorer重启安全性、GAC部署安全性和单实例升级行为。
- **新增** 特定回归测试：验证标签文本布局和字段阴影问题的修复。
- 本地运行：推荐先使用 MSBuild 构建测试工程，再执行 dotnet test。

**更新** 测试策略现在包括专门的架构治理测试层，用于监控和防止架构债务增长。架构治理测试通过量化指标如代码行数、嵌套控制器数量、所有者回引用计数等来确保架构质量的持续改进，同时维持 687/687 绿色测试基线。
...
### 架构治理测试架构
架构治理基础设施包含两个核心测试类，用于监控和防止架构债务增长：

#### ArchitectureHotspotBudgetTests 架构热点预算测试
- **NoGrowth_Budget_Is_Not_Exceeded**：立即执行的无增长门禁，确保架构指标不超出基线
- **Final_Budget_QTTabBarClass_FileLines**：最终预算测试，限制QTTabBarClass.cs文件行数不超过500行
- **Final_Budget_QTTabBarClass_NestedControllers**：最终预算测试，要求嵌套控制器数量为0
- **Final_Budget_QTTabBarClass_OwnerBackReferences**：最终预算测试，要求_owner回引用计数为0
- **Final_Budget_QTButtonBar_FileLines**：最终预算测试，限制QTButtonBar.cs文件行数不超过450行
- **Final_Budget_OptionsDialog_FileLines**：最终预算测试，限制OptionsDialog.xaml.cs文件行数不超过500行

#### StructuralGovernanceBaselineTests 结构治理基线测试
- **Current_Debt_Baseline_Is_Not_Exceeded**：验证当前架构债务不超过基线值
- **Architecture_Plan_Reopens_W10_And_C6**：验证架构计划文档中标记W10和C6问题为REOPENED状态
- **SourceMetrics 工具类**：提供源代码度量的静态方法集合

```mermaid
flowchart TD
A["架构治理基础设施"] --> B["ArchitectureHotspotBudgetTests"]
A --> C["StructuralGovernanceBaselineTests"]
B --> D["NoGrowth_Budget_Is_Not_Exceeded"]
B --> E["Final_Budget_QTTabBarClass_FileLines"]
B --> F["Final_Budget_QTTabBarClass_NestedControllers"]
B --> G["Final_Budget_QTTabBarClass_OwnerBackReferences"]
B --> H["Final_Budget_QTButtonBar_FileLines"]
B --> I["Final_Budget_OptionsDialog_FileLines"]
C --> J["Current_Debt_Baseline_Is_Not_Exceeded"]
C --> K["Architecture_Plan_Reopens_W10_And_C6"]
C --> L["SourceMetrics 工具类"]
L --> M["FamilyLines 方法"]
L --> N["NestedControllerCount 方法"]
L --> O["TokenCount 方法"]
L --> P["PartialDeclarationCount 方法"]
L --> Q["FileLines 方法"]
L --> R["ReadRepoFile 方法"]
```
...
### 源代码度量工具
SourceMetrics 工具类提供了全面的源代码度量功能：

#### 家族代码行数统计
- **FamilyLines 方法**：统计指定类家族的所有文件行数总和
- **文件匹配模式**：使用 familyName + "*.cs" 模式匹配相关文件
- **递归统计**：遍历所有匹配文件并累加行数

#### 嵌套控制器计数
- **NestedControllerCount 方法**：通过反射统计QTTabBarClass的嵌套类型数量
- **反射访问**：使用 BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Instance
- **类型筛选**：仅统计嵌套类型，排除普通成员类型

#### 令牌计数统计
- **TokenCount 方法**：统计指定令牌在代码中的出现次数
- **字符串搜索**：使用 IndexOf 方法进行精确匹配
- **全文扫描**：遍历所有匹配文件并统计令牌出现次数

#### 部分类声明计数
- **PartialDeclarationCount 方法**：统计部分类声明的数量
- **正则表达式匹配**：使用 partial\s+class\s+familyName 模式
- **多文件扫描**：遍历所有匹配文件并统计声明数量

#### 仓库文件访问
- **ReadRepoFile 方法**：读取仓库中的任意文件内容
- **相对路径支持**：支持相对于仓库根的相对路径
- **文件存在性检查**：自动定位仓库根目录

#### 仓库根目录定位
- **RepoRoot 方法**：通过查找解决方案文件自动定位仓库根目录
- **向上遍历**：从当前目录向上遍历直到找到解决方案文件
- **异常处理**：找不到仓库根时抛出 InvalidOperationException

### 架构治理指标
架构治理测试监控以下关键指标：

#### 代码复杂度指标
- **QTTabBarClass 家族行数**：限制不超过7283行
- **QTButtonBar 家族行数**：限制不超过2164行
- **嵌套控制器数量**：限制不超过25个
- **_owner 回引用计数**：限制不超过1397个

#### 最终目标指标
- **QTTabBarClass.cs 文件行数**：限制不超过500行
- **QTButtonBar.cs 文件行数**：限制不超过450行
- **OptionsDialog.xaml.cs 文件行数**：限制不超过500行
- **嵌套控制器数量**：目标为0个
- **_owner 回引用计数**：目标为0个
- **部分类声明数量**：限制不超过4个
...
1. **架构债务监控**：通过量化指标监控代码复杂度增长
2. **重构质量保证**：确保架构重构不会导致代码复杂度反弹
3. **持续改进保障**：通过基线测试确保架构质量持续提升
4. **团队协作规范**：为团队提供明确的架构质量标准
5. **CI/CD 集成**：支持在持续集成环境中自动执行架构质量检查

**更新** 架构治理基础设施为项目的长期架构健康提供了强有力的保障，通过量化指标和自动化测试确保架构债务不会增长，同时为未来的架构改进提供了明确的指导方向。
...
- **新增** 架构治理测试集成：CI 流程包含架构治理测试执行，验证架构债务不增长。
...
participant ARCHGOV as "架构治理测试<br/>架构债务监控"
...
ARCHREM-->>CI : 测试结果
CI->>ARCHGOV : 运行架构治理测试<br/>验证架构债务不增长
ARCHGOV-->>CI : 测试结果
...
- **新增** 架构治理测试调试：
  - 架构债务增长：检查 SourceMetrics.FamilyLines 和 SourceMetrics.NestedControllerCount 的返回值
  - 基线测试失败：验证架构指标是否超过预设的基线值
  - 最终预算测试失败：检查代码重构是否达到了最终的架构目标
  - 仓库根目录定位问题：验证 RepoRoot 方法能否正确找到解决方案文件
  - 文件路径问题：检查相对路径是否正确指向目标文件
  - 反射访问问题：验证嵌套类型统计的准确性
- **新增** 安装程序互操作性测试调试：
...
**更新** 增加了架构治理测试的调试指导，包括架构债务监控、基线测试、最终预算测试等的调试指导。特别强调了SourceMetrics工具类的使用方法、仓库根目录定位、反射访问验证等架构治理相关的调试技巧。
...
- **新增** 架构治理测试要求：涉及架构重构的重大变更必须添加相应的架构治理测试，确保架构债务不增长，包括代码行数、嵌套控制器数量、所有者回引用计数等指标的监控。
...
**更新** 强调了架构治理测试在重大架构重构过程中的重要性，要求所有相关的架构重构都必须包含相应的架构治理测试用例，确保架构债务不会增长。新增了架构治理测试的要求，包括代码行数限制、嵌套控制器数量控制、所有者回引用计数监控等关键指标的验证。
...
**更新** 新增的架构治理基础设施为项目开发提供了更加全面的质量保障。架构治理测试通过量化指标监控代码复杂度增长，确保架构债务不会累积，同时为未来的架构改进提供了明确的指导方向。

通过这些改进，开发者可以更加自信地进行架构重构和功能扩展，同时保持项目的长期可维护性、安全性和用户体验。架构治理基础设施为现代C#架构重构提供了实用的验证框架，确保所有架构改进都经过严格验证。

**新增** 架构治理基础设施提供的专业测试用例，为代码复杂度控制、嵌套控制器数量限制、所有者回引用计数监控等架构改进提供了全面的验证保障。这些测试用例涵盖了架构债务监控、基线测量、最终预算验证等关键功能，为项目的架构质量提供了强有力的保障。

通过这些改进，项目建立了更加完善的质量保障体系，从架构重构到UI修复，从安全性增强到用户体验改进，都得到了全面的测试覆盖和验证保障。架构治理基础设施为现代C#架构重构提供了实用的验证框架，确保所有架构改进都经过严格验证。

**更新** 通过这些改进，项目维持了 687/687 绿色测试基线，确保了所有架构重构和改进的完整性和正确性。新增的架构治理基础设施为项目的长期可维护性和稳定性提供了强有力的保障。