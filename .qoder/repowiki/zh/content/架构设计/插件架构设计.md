...
- [IPluginServerHost.cs](file://QTTabBar/Plugin/IPluginServerHost.cs)
</cite>

## 更新摘要
**所做更改**   
- 更新了插件服务器架构部分，反映 PluginServer 从 QTTabBarClass 内部嵌套类重构为顶级密封类
- 新增了依赖注入机制说明，通过 IPluginServerHost 和 IPluginServerTabHost 接口实现解耦
- 完全消除了所有 123 个 _owner 引用，替换为 _host 接口调用
- 增强了插件系统架构的模块化和可维护性分析
- 更新了架构图以显示新的依赖注入模式
...
**更新** 插件服务器已重构为顶级密封类，采用依赖注入模式，通过 IPluginServerHost 接口消除循环依赖
...
- [IPluginServerHost.cs:1-42](file://QTTabBar/Plugin/IPluginServerHost.cs#L1-L42)
...
- **新增** IPluginServerHost：插件服务器的宿主接口，通过依赖注入提供宿主能力访问。
...
class IPluginServerHost {
+ExplorerHandle : IntPtr
+IsHandleCreated : bool
+Handle : IntPtr
+ShellBrowser : ShellBrowserEx
+OpenGroup(groupName, fForceNewWindow)
+NavigateToIndex(fBack, index) bool
+CloseTab(tab) bool
+TryCreateTab(address, index, locked, select) bool
}
class PluginManager {
+Initialize()
+LoadAssembly(path)
...
IPluginClient <.. PluginServer : "Open(this,shellBrowser)"
IPluginServer <.. PluginServer : "实现"
IPluginServerHost <.. PluginServer : "依赖注入"
PluginManager --> PluginServer : "协调生命周期"
PluginServer <.. PluginServer_Commands : "部分类"
PluginServer <.. PluginServer_Lifetime : "部分类"
PluginServer <.. PluginServer_TabAccess : "部分类"
```
...
- **新增** IPluginServerHost：通过依赖注入提供宿主能力访问，完全替代了原有的 _owner 字段访问模式。
...
participant HostInterface as "IPluginServerHost"
...
PM->>HostInterface : 创建 PluginServer(host)
HostInterface-->>PS : 注入宿主接口
...
- [PluginServer.cs:48-61](file://QTTabBar/PluginServer.cs#L48-L61)
...
**更新** 命令分发逻辑现已移至独立的 PluginServer.Commands.cs 文件中，并通过依赖注入接口访问宿主能力：
...
PS_Cmd->>HostInterface : _host.Explorer.Refresh()
HostInterface-->>PS_Cmd : 执行成功
PS_Cmd-->>PL : 返回true
```
...
**重大更新** 插件服务器已从 QTTabBarClass 内部嵌套类重构为顶级密封类，采用依赖注入模式：

#### 主类文件 (PluginServer.cs)
- 包含 PluginServer 类的核心定义和字段
- 事件声明和基础属性
- 构造函数接受 IPluginServerHost 接口进行依赖注入
- 事件触发方法（OnTabChanged, OnSelectionChanged 等）
- 本地化字符串管理和菜单注册
- **关键改进**：使用 `_host` 字段替代原来的 `_owner` 字段，通过接口访问宿主能力
...
- 处理导航、窗口操作、UI 控制等命令
- 提供统一的命令执行入口
- **关键改进**：所有命令执行都通过 `_host` 接口调用，消除了循环依赖
...
- 管理插件实例的注册和注销
- 处理过滤器插件的特殊初始化逻辑
- **关键改进**：在 Dispose 方法中正确释放 `_host` 引用，防止内存泄漏
...
- 处理标签页的创建、删除、选择和操作
- 管理标签页的状态和属性
- **关键改进**：TabWrapper 也使用 IPluginServerHost 接口，完全消除了对宿主类型的直接依赖
...
IPluginServerHost <.. PluginServer_Main : "依赖注入"
IPluginServerHost <.. PluginServer_Commands : "依赖注入"
IPluginServerHost <.. PluginServer_Lifetime : "依赖注入"
IPluginServerHost <.. PluginServer_TabAccess : "依赖注入"
...
- **关键改进**：PluginServer 通过 IPluginServerHost 接口与宿主解耦，完全消除了循环依赖。

**更新** 重构后的依赖关系更加清晰，采用依赖注入模式：

```mermaid
graph LR
PM["PluginManager"] --> |反射/校验| PL["插件(IPluginClient)"]
PS_Main["PluginServer.Main"] --> |事件/命令| PL
PS_Cmd["PluginServer.Commands"] --> |命令执行| HostInterface["IPluginServerHost"]
PS_Life["PluginServer.Lifetime"] --> |生命周期| PL
PS_Tab["PluginServer.TabAccess"] --> |标签操作| HostInterface
HostInterface --> |实现| Host["宿主(QTTabBarClass)"]
PL --> |订阅事件| PS_Main
PL --> |调用命令| PS_Cmd
...
- [PluginManager.cs:299-328](file://QTTabBar/PluginManager.cs#L299-328)
...
- 依赖注入问题：检查 IPluginServerHost 接口的实现是否正确
...
## 结论
QTTabBar-Next 的插件架构以清晰的接口契约为基础，结合严格的来源与签名校验，实现了高内聚、低耦合的扩展生态。**经过重大重构后，插件服务器采用了更加先进的架构设计，从 QTTabBarClass 内部嵌套类重构为顶级密封类，并通过依赖注入模式完全消除了循环依赖**。通过引入 IPluginServerHost 接口，所有 123 个 _owner 引用都被替换为 _host 接口调用，显著提高了系统的稳定性和可维护性。

**重构带来的核心优势**：
- **架构解耦**：通过依赖注入彻底消除了循环依赖，提高了系统的可测试性和可维护性
...
- **内存管理**：依赖注入模式避免了循环引用导致的内存泄漏问题

开发者只需遵循约定与最佳实践，即可快速构建高质量的插件，同时享受更稳定、更可维护的插件生态系统。