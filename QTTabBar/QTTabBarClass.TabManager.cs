    ...
    public partial class QTTabBarClass {
        ...
        internal class TabManager {
            ...
            public void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
            {
                ...
                QTabItem selectedTab = _owner.tabControl1.SelectedTab;
                string currentPath = selectedTab.CurrentPath;
                if (_owner.IsSpecialFolderNeedsToTravel(currentPath) &&
                   _owner.LogEntryDic.ContainsKey(selectedTab.GetLogHash(true, 0)))
                {
                    _owner.NavigatedByCode = true;
                    _owner.CurrentTab = selectedTab;
                    while (_owner.lstActivatedTabs.Remove(_owner.CurrentTab))
                    {
                    }
                    ...
                    _owner.fNavigatedByTabSelection = _owner.NavigateToPastSpecialDir(_owner.CurrentTab.GetLogHash(true, 0));
                    if (_owner.pluginServer != null)
                    {
                        _owner.pluginServer.OnTabChanged(_owner.tabControl1.SelectedIndex, selectedTab.CurrentIDL, selectedTab.CurrentPath);
                    }
                ...
                else
                {
                    ...
                    using (idlw)
                    {
                        ...
                        if (((currentPath != _owner.CurrentAddress) ||
                            (QTUtility.IsXP && (currentPath == QTUtility.PATH_SEARCHFOLDER))) ||
                           _owner.NowTabCloned)
                        {
                            _owner.NavigatedByCode = true;
                            _owner.fNavigatedByTabSelection = true;
                            _owner.NowTabCloned = false;
                            if (_owner.ShellBrowser.Navigate(idlw) != 0)
                            {
                                CancelFailedTabChanging(currentPath);
                                return;
                            }
                    ...
                    if (_owner.pluginServer != null)
                    {
                        _owner.pluginServer.OnTabChanged(_owner.tabControl1.SelectedIndex, _owner.CurrentTab.CurrentIDL, _owner.CurrentTab.CurrentPath);
                    }
                }
            }
            ...
            public void SaveSelectedItems(QTabItem tab)
            {
                Address[] addressArray;
                string str;
                if (
                    ((tab != null) && !string.IsNullOrEmpty(_owner.CurrentAddress)) &&
                    _owner.ShellBrowser.TryGetSelection(out addressArray, out str, false, _owner.ShellBrowser))
                {
                    if (addressArray != null && addressArray.Length > 0)
                    {
                        QTUtility2.log("SaveSelectedItems addressArray " + addressArray[0].Path);
                    }
                    tab.SetSelectedItemsAt(_owner.CurrentAddress, addressArray, str);
                }
            }

            public void tabControl1_Selecting(object sender, QTabCancelEventArgs e)
            {
                if (_owner.NowTabsAddingRemoving)
                {
            ...
            public void tabControl1_PlusButtonClicked(object sender, QTabCancelEventArgs e) {
                ...
                if (File.Exists(clipPath))
                {
                    try
                    {
                        ...
                        _owner.ShellBrowser.TrySetSelection(new Address[] { new Address(selectMe) }, null, true);
                    }
                    catch (Exception e1)
                    {
