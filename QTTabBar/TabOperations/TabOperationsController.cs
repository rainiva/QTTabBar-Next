    ...
    internal sealed class TabOperationsController {
        ...
        public void AddStartUpTabs(string openingGRP, string openingPath) {
            ...
            foreach(string path in GroupsManager.Groups.Where(g => g.Startup && openingGRP != g.Name).SelectMany(g => g.Paths)) {
                ...
                if(_host.TryCreateTabAtPosition(new Address(path), TabPos.Rightmost, false, false, false)) {
                    QTabItem tabPage = _host.tabControl1.TabPages
                        .Cast<QTabItem>()
                        .LastOrDefault(item => path.PathEquals(item.CurrentPath));
                    if(tabPage != null) {
        ...
        internal void OpenNewWindow(IDLWrapper idlwGiven) {
            ...
            using(IDLWrapper idlwLink = idlwGiven.ResolveTargetIfLink()) {
                ...
                if(_host.ShellBrowser.Navigate(idlw, wFlags) != 0) {
                    ...
                    WindowCaptureSession.ClearGroup();
        ...
        public void OpenGroup(string groupName, bool fForceNewWindow, bool fDisableOverrides = false) {
            ...
            if (fForceNewWindow) {
                ...
                WindowCaptureSession.EnqueueGroup(groupName);
                ...
                try {
                    ...
                    foreach (var gpath in gpaths) {
                        ...
                        if (!flag3 || !list.Contains(gpath.ToLower())) {
                            if (_host.TryCreateTabAtPosition(
                                    new Address(gpath),
                                    Config.Tabs.NewTabPosition,
                                    false,
                                    false,
                                    true)) {
                                num++;
                                flag = true;
                                if (tabPage == null) {
                                    tabPage = _host.tabControl1.TabPages
                                        .Cast<QTabItem>()
                                        .LastOrDefault(item => gpath.PathEquals(item.CurrentPath));
                                }
                            }
                    ...
                    if (condition) {
                        ...
                        flag5 = tabPage != _context.CurrentTab;
        ...
        internal void CloneCurrentTab(bool fSelect = true) {
            CloneTabButton(_context.CurrentTab, null, fSelect, -1);
        }
