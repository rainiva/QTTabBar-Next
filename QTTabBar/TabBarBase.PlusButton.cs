    ...
    public abstract partial class TabBarBase {
        internal void tabControl1_PlusButtonClicked(object sender, QTabCancelEventArgs e) {
            ...
            if(File.Exists(clipPath)) {
                try {
                    QTLogger.log("tabControl1_PlusButtonClicked file exist " + clipPath);
                    ...
                    QTLogger.MakeErrorLog(e1, "tabControl1_PlusButtonClicked for file:" + clipPath);
                    openDefault();
                }
            }
            else if(Directory.Exists(clipPath)) {
                try {
                    QTLogger.log("tabControl1_PlusButtonClicked Directory exist " + clipPath);
                    OpenNewTab(clipPath, blockSelecting, fForceNew);
                }
                catch(Exception e1) {
                    QTLogger.MakeErrorLog(e1, "tabControl1_PlusButtonClicked for director :" + clipPath);
                    openDefault();
                }
            }
            else {
        ...
        internal void openDefault() {
            ...
            if(!isOpend) {
                ...
                using(IDLWrapper w = new IDLWrapper(idl)) {
                    QTLogger.log("tabControl1_PlusButtonClicked MyComputer ");
