        ...
        public bool DoBindAction(BindAction action, bool fRepeat = false, QTabItem tab = null, IDLWrapper item = null) {
            ...
            switch(action) {
                ...
                case BindAction.ShowToolbarMenu: // hmm.
                    ...
                    _uiHost.ContextMenuSys.Show(_uiHost.PointToScreen(new Point(tabRect.Right + 10, tabRect.Bottom - 10)));
                    break;

                case BindAction.ShowTabMenuCurrent:
                    if(tab.Index != -1) {
                        _host.ContextMenuedTab = tab;
                        Rectangle rect = _host.TabControl.GetTabRect(tab.Index, true);
                        _uiHost.ContextMenuTab.Show(_uiHost.PointToScreen(new Point(rect.Right + 10, rect.Bottom - 10)));
                    }
                    break;

                case BindAction.ShowTabMenu:
                    _host.ContextMenuedTab = tab;
                    _uiHost.ContextMenuTab.Show(Cursor.Position);
                    break;
                ...
                case BindAction.CopySelectedPaths:
                    if(_uiHost.ListView.SubDirTipMenuIsShowing() || (_uiHost.SubDirTipTab != null && _uiHost.SubDirTipTab.MenuIsShowing)) {
                        return false;
                    }
                    _uiHost.DoFileTools(0);
                    break;
                ...
                case BindAction.TransparencyMinus: {
                        ...
                        int num10;
                        byte num11;
                        if(0x80000 != ((int)PInvoke.Ptr_OP_AND(PInvoke.GetWindowLongPtr(_uiHost.ExplorerHandle, -20), 0x80000))) {
                            if(action == BindAction.TransparencyPlus) {
                                return true;
                            }
                            PInvoke.SetWindowLongPtr(_uiHost.ExplorerHandle, -20, PInvoke.Ptr_OP_OR(PInvoke.GetWindowLongPtr(_uiHost.ExplorerHandle, -20), 0x80000));
                            PInvoke.SetLayeredWindowAttributes(_uiHost.ExplorerHandle, 0, 0xff, 2);
                        }
                        if(PInvoke.GetLayeredWindowAttributes(_uiHost.ExplorerHandle, out num9, out num11, out num10)) {
                           // IntPtr ptr3;
                            if(action == BindAction.TransparencyPlus) {
                                if(num11 > 0xf3) {
                                    num11 = 0xff;
                            ...
                            PInvoke.SetLayeredWindowAttributes(_uiHost.ExplorerHandle, 0, num11, 2);
                            // IM!
                            ...
                            //}
                            if(num11 == 0xff) {
                                PInvoke.SetWindowLongPtr(_uiHost.ExplorerHandle, -20, PInvoke.Ptr_OP_AND(PInvoke.GetWindowLongPtr(_uiHost.ExplorerHandle, -20), 0xfff7ffff));
                            }
                        }
                    }
                    break;
                ...
                case BindAction.ShowSDTSelected:
                    ...
                    _uiHost.ListView.ShowAndClickSubDirTip();
                    break;

                case BindAction.SendToTray:
                    _uiHost.MinimizeToTray();
                    break;
                ...
                case BindAction.NewFile:
                    _uiHost.CreateNewFile();
                    break;

                case BindAction.MergeWindows:
                    _uiHost.MergeAllWindows();
                    break;
