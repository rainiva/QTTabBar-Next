...
namespace QTTtabBarTests {
    ...
    public class ArchitectureReviewRemediationTests {
        ...
        [Test]
        public void HookInputController_EnableApiHook_Does_Not_Reinitialize_HookLib() {
            ...
            Assert.IsFalse(body.Contains("HookLibManager.Initialize()"),
                "EnableApiHook should rely on InitializationOrchestrator for HookLibManager.Initialize");
        }
        ...
        [Test]
        public void QTUtility_getNightMode_Opens_Personalize_Key_ReadOnly() {
            string content = ReadQtTabBarFile("QTUtility.cs");
            int methodIndex = content.IndexOf("bool getNightMode()", StringComparison.Ordinal);
            Assert.GreaterOrEqual(methodIndex, 0);
            int searchEnd = Math.Min(content.Length, methodIndex + 800);
            string body = content.Substring(methodIndex, searchEnd - methodIndex);
            Assert.IsTrue(body.Contains("OpenSubKey(REG_PERSONALIZE, false)"),
                "getNightMode should open AppsUseLightTheme registry key read-only");
        }
        ...
        public void InitializationOrchestrator_Assigns_TextResourcesDic_Under_SyncRoot() {
            string content = ReadQtTabBarFile("InitializationOrchestrator.cs");
            Assert.IsTrue(content.Contains("lock(QTUtility.syncRoot)"),
                "InitializationOrchestrator should publish TextResourcesDic under syncRoot");
            Assert.IsTrue(
